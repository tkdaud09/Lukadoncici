...
public class MemberCastApp {
	public static void main(String[] args) {
		...
		// => 부모클래스의 생성자로 부모클래스 객체를 생성하고 자식클래스의 생성자로 자식
		//클래스 객체를 생성하여 자식클래스의 참조변수에 자식클래스의 객체 저장 - 가능
		...
		//부모클래스의 객체 저장 - 불가능(에러 발생)
		//MemberEvent member3=new Member(); >> member클래스에 (아이디 이름) 있음; 하지만 이메일이 없어서 오류
		System.out.println("==============================================================");
		...
		//★명시적 객체 형변환 : Cast 연산자를 사용하여 부모클래스 참조변수의 자료형을 자식
		//클래스로 변경하면 일시적으로 참조변수에 자식클래스의 객체가 자동 저장
		// => 명시적 객체 형변환에 의해 자식클래스로 자료형이 변경된 참조변수는 자식클래스의
		//객체가 저장되므로 자식클래스의 메소드 호출 가능
		/*
		...
		*/
		...
		//자료형을 자동으로 자식클래스로 변경하여 일시적으로 자식클래스의 객체가 참조변수에
		//저장되어 자식클래스의 메소드 호출
		member4.display();
		System.out.println("==============================================================");
	}
