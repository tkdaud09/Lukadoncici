...
public class CalcAgeApp {
	public static void main(String[] args) {
		...
		String name=scanner.nextLine(); //String은 nextLine
		
		System.out.print("태어난 년도 >> ");
		int birthYear=scanner.nextInt(); //숫자는 nextInt
		
		//java.util.Date 클래스로 객체를 생성하여 참조변수에 저장
		// => Date 클래스 : 날짜와 시간을 저장하기 위한 클래스
		// => Date 클래스의 기본생성자를 사용하여 객체를 생성하면 시스템의 현재 날짜와 
		//시간이 저장된 Date 객체가 생성
		Date now=new Date();
		
		//Date.getYear() : Date 객체에 저장된 날짜와 시간에서 년도를 정수값으로 반환하는 메소드
		// => 1900년을 기준으로 1년에 1씩 증가된 정수값 반환
		...
		//int age=2023-birthYear+1; // 다음년도에는 못씀
