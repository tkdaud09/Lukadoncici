...
public class Student {
	//인스턴스 필드(Instance(객체) Field) : 객체가 생성될 때 메모리(HeapArea)에 생성되는 필드
	private int num;
	private String name;
	private int kor, eng, tot;
	
	//정적 필드 (static) : 클래스를 읽어 메모리(MethodArea)에 저장될 때 생성되는 필드 (객체와 상관xxx)
	// => 객체가 생성되기 전에 메모리에 하나만 생성되는 필드
	// => 생성자에서 초기화 처리하지 않고 직접 초기값을 필드에 저장
