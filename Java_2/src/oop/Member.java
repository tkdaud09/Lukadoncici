...
// => 메소드 내부에서 객체를 대신 표현하기 위해 사용하는 키워드
// => this 키워드를 사용하지 않아도 자동으로 객체의 필드 또는 메소드에 접근하여 사용

// this.id = id; >> this(내가).(가지고있는)

// 메소드에 this 키워드가 존재해야되는 이유
// => 필드는 객체마다 메모리(HeapArea)에 독립적으로 생성되지만 메소드는 객체와 상관없이
//    메모리(MethdArea)에 하나만 생성 - 프로토타입 클래스(Prototype class)
// => ★메소드에서 필드를 사용할 때 this 키워드로 객체를 명확히 구분하여 필드에 접근하도록 사용

//★this 키워드를 사용하는 경우★
//1.메소드에서 매개변수의 이름을 필드의 이름과 동일하게 작성한 경우 필드를 표현하기 위해
//this 키워드 사용 - 생성자 및 Setter 메소드
//2.생성자에서 다른 생성자를 호출하여 초기화 작업을 하기위해 this 키워드 사용 -거의 안씀
//3.이벤트 처리 프로그램 또는 다중 스레드 프로그램의 메소드에서 객체 자체를 표현하기 위헤
//this 키워드 사용
