...
//Spring 프레임워크에서는 BeanFactory 객체 또는 ApplicationContext 객체로 스프링 컨테이너(Spring Container) 기능 제공
// => 스프링 컨테이너는 환경설정파일(Spring Bean Configuration File - XML)로 클래스를 제공받아 Spring Bean(객체) 관리
...
public class CreateBeanApp {
	public static void main(String[] args) {
		...
		// => BeanFactory 객체를 생성할 때 Spring Bean Configuration File을 제공받아 스프링 컨테이너 생성 - 스프링 컨테이너 초기화 작업 
		// => 매개변수에는 Spring Bean Configuration File의 경로를 제공받아 Resource 객체로 생성하여 전달
		// => BeanFactory 객체는 Spring Bean Configuration File에 등록된 클래스로 미리 객체를 생성하지 않고 Spring Bean 요청시 생성하여 제공
		BeanFactory factory=new XmlBeanFactory
				(new FileSystemResource("src/main/resources/04-1_beanCreate.xml"));
		System.out.println("=============== Spring Container 초기화 후 ===============");
		//BeanFactory.getBean(String beanName) : 매개변수로 Spring Bean를 구분하기 위한 
		//식별자(beanName)을 전달받아 스프링 컨테이너로부터 Spring Bean(객체)를 생성하여 반환하는 메소드
