...
<body>
	...
	<%-- => choose 태그에 하위태그를 제외한 다른 코드가 존재할 경우 에러 발생 - JSP 주석 예외 --%>
	<c:choose>
		<%-- when 태그 : 조건에 대한 결과로 태그내용의 포함 여부를 선택하기 위한 태그 --%>
		<%-- test 속성 : false 또는 true 중 하나를 속성값으로 설정 - 태그 속성값으로 EL 사용 가능 --%>
		<%-- => test 속성값이 [true]인 경우 태그내용을 포함하고 choose 태그 종료 --%>
		...
		<%-- otherwise 태그 : when 태그의 모든 조건이 거짓인 경우 태그내용을 무조건 포함시키기 위한 태그  --%>
		<c:otherwise>
			<p>지구로 이동합니다.</p>
		</c:otherwise>
	</c:choose>
