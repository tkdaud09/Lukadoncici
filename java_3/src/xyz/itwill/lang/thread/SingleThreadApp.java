...
//스레드(Thread) : 프로세스에서 명령을 실행하기 위한 최소의 작업단위 - 프로그램의 흐름
...
public class SingleThreadApp {
	public static void main(String[] args) {
		System.out.println("SingleThreadApp 클래스의 amin() 메소드 시작");
		
		//Thread 클래스 : 스레드 관련 정보를 저장하여 스레드 관련 제어 기능을 메소드로
		//제공하기 위한 클래스
		//Threa.currentThread() : 현재 명령을 읽어 처리하는 스레드에 대한 
		//Thread 객체를 반환하는 메소드
		//Thread.getName() : Thread 객체된 저장된 스레드의 이름(고유값)을 반환하는 메소드 
		System.out.println("["+Thread.currentThread().getName()+"] 스레드에 의해 main()메소드의 명령 실행");
		
		//객체를 생성하여 메소드를 호출한 경우 스레드가 메소드로 이동하여 메소드의 명령을 실행
