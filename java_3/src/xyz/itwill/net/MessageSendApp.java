...
//UDP 프로그램 : DatagramSocket 클래스와 DatagramSocket 클래스를 이용하여 작성 (비연결형 프로그램)
...
public class MessageSendApp {
	public static void main(String[] args) throws IOException {
		...
		DatagramPacket packet=new DatagramPacket(date,  date.length, address, 4000);
		
		//DatagramSocket.send(DatagramPacket packet) : 매개변수로 전달받은 DatagramPacket 객체의
		//패킷정보를 이용하여 데이터(패킷)를 전달하는 메소드
		socket.send(packet);
