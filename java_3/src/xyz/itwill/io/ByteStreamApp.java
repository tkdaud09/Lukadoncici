...
public class ByteStreamApp {
	public static void main(String[] args) throws IOException {
		...
		while(true) { //~하는 동안
			...
			//InputStream.read() : 입력스트림에 존재하는 값을 원시데이터(1Byte)로 읽어 
			//정수값(int)으로 반환하는 메소드 
			// => 입력스트림에 값이 없는 경우 스레드는 일시중시 상태로 전환
			...
			// => 출력스트림에 값을 전달하면 모니터 출력
			//OutpuyStream.write(int b) : 매개변수로 제공받은 값을 원시데이터(1Byte)로 
			//출력스트림에 전달하는 메소드
