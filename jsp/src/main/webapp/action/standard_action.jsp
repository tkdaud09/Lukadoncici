...
<body>
	...
	<hr>
	<!-- include 태그 매우 중요!!  forward도 중요 -->
	<p>include 태그 : JSP 문서의 스레드를 다른 JSP 문서로 이동하여 실행된 결과(웹문서)를
	제공받아 태그를 사용한 위치에 포함하는 태그</p>
	<p>=>include 태그를 사용하면 다른 JSP 파일의 내용을 현재 JSP 파일에 포함 > 
	포함된 JSP 파일의 내용은 서버 측에서 처리되어 클라이언트에게 전달</p>
	<p>forward 태그 : JSP 문서의 스레드를 다른 JSP 문서로 이동하여 실행된 결과(웹문서)를
	클라이언트에게 전달하여 응답하는 태그</p>
	<p>=>forward 태그를 사용하면 현재 JSP 페이지의 실행 흐름을 다른 JSP 페이지로 전달 >
	 즉, 클라이언트의 요청을 다른 JSP 페이지로 전달하여 그 결과를 클라이언트에게 응답</p> 
	<p>param 태그 : JSP 문서에서 스레드가 이동된 JSP 문서로 값을 전달하기 위한 태그
	- include 태그와 forward 태그의 하위태그로 실행</p>
	<p>=>태그를 사용하여 전달된 값은 이동된 JSP 문서에서 파라미터로 사용</p>
	<p>useBean 태그 : JSP 문서에서 사용할 수 있는 객체를 제공하기 위한 태그(DAO 유사)
	- 내장객체에 저장된 속성값을 반환받아 제공하거나 새로운 객체를 생성하여 제공</p>
	<p>=>JavaBeans와 관련하여 사용되며, 해당 JSP 페이지에서 사용할 객체를 불러오거나 생성</p>
