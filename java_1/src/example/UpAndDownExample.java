...
//시작 > 난수값 저장 > 10번!(for) 횟수 정해짐 > 1~100 (while) 횟수 안정해짐 
// > 비교 > 같으면 횟수출력 > break > 다르면 크다작다 출력 > 다시 위로 > 10번이내 x 난수값 출력 
...
public class UpAndDownExample {
	public static void main(String[] args) {
		...
		//정답 상태를 저장하기 위한 변수
		// => false : 정답을 맞추지 못한 상태, true : 정답을 맞춘상태
		...
		for(int cnt=1;cnt<=10;cnt++) {
			...
			if(dap == input) {
				...
				System.out.println("[결과]"+input+"보다 큰 값을 작성해보세요");
			} else {
				System.out.println("[결과]"+input+"보다 작은 값을 작성해보세요");
			}
		}
