...
public class StudentDAO {
	...
	public List<Student> selectStudentList() {
		...
		try {
			//SqlSession.selectList(String elemetId) : 매퍼에 등록된 SELECT 명령을 제공받아
			//DBMS 서버에 전달하여 실행하고 실행결과를 List 객체로 반환하는 메소드
			// => elementId 매개변수에는 매퍼의 식별자(mapper 엘리먼트의 namespace 속성값)와
			//select 엘리먼트의 식별자(id 속성값)을 이용한 무자열을 전달받아 매퍼에 등록된 SQL 명령을
			//DBMS 서버에 전달하여 실행하고 실행결과를 Java 객체로 매핑 처리하여 반환
